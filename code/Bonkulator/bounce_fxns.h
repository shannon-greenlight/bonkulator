...
String check_bounce()
{
    if (bounce_triggered && millis() > bounce_next_time)
    {
        ...
        uint16_t dac_val = (bounce_reading * output_scale * 1.16) + 2048 + output_offset + bounce_offset_correction;
        ...
        if (bounce_triggered)
        {
            trigger_bounce(bounce_triggered_by[the_bounce_input], TRIGGER_ACTIVE);
        }
        return "  CV" + String(the_bounce_input) + " Reading: " + String(reading) + "    Triggered by: T" + String(bounce_triggered_by[the_bounce_input]) + "  ";
    }
    return "";
}
